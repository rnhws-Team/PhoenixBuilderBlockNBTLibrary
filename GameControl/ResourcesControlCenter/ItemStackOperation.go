...
func (i *itemStackReuqestWithResponce) WriteRequest(
	...
	_, exist := i.requestWithResponce.Load(key)
	if exist {
		return fmt.Errorf("WriteRequest: %v has already existed", key)
	}
	// if key has already exist
	i.requestWithResponce.Store(
		key,
		singleItemStackRequestWithResponce{
...
func (i *itemStackReuqestWithResponce) LoadResponceAndDelete(key int32) (protocol.ItemStackResponse, error) {
	value, exist := i.requestWithResponce.Load(key)
	if !exist {
		return protocol.ItemStackResponse{}, fmt.Errorf("LoadResponceAndDelete: %v is not recorded", key)
	}
	// if key is not exist
	get, normal := value.(singleItemStackRequestWithResponce)
	if !normal {
		return protocol.ItemStackResponse{}, fmt.Errorf("tryToWriteResponce: Failed to convert value into singleItemStackRequestWithResponce; value = %#v", value)
	}
	// convert data
	ans := <-get.resp
	i.requestWithResponce.Delete(key)
	return ans, nil
	// return
}
...
func (i *itemStackReuqestWithResponce) updateItemData(
	...
	value, exist := i.requestWithResponce.Load(resp.RequestID)
	if !exist {
	...
	get, normal := value.(singleItemStackRequestWithResponce)
	if !normal {
		panic(fmt.Sprintf("updateItemData: Failed to convert value into singleItemStackRequestWithResponce; value = %#v", value))
	}
	// convert data
